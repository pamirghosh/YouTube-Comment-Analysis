...
def createSummery(comments):
    ...
    extracted_text = re.search(r'\[BEGINNING\](.*?)\[END\]', response['text'], re.DOTALL).group(1).strip()
...
if __name__ == "__main__":
    ...
    if st.session_state.analysis_done and not st.session_state.summary_done:
        summary_button_placeholder = st.empty()  # Placeholder for the Summary button
        spinner_placeholder = st.empty()         # Placeholder for the spinner and summary generation message
        summary_heading_placeholder = st.empty() # Placeholder for the summary heading
        summary_text_placeholder = st.empty()    # Placeholder for the summary text

        if summary_button_placeholder.button("Summary"):
            ...
            summary_heading_placeholder.markdown("<h3>Summary of Negative Comments:</h3>", unsafe_allow_html=True)
            summary_text_placeholder.text(st.session_state.summary_text)

    elif st.session_state.summary_done:
        st.markdown("<h3>Summary of Negative Comments:</h3>", unsafe_allow_html=True)
